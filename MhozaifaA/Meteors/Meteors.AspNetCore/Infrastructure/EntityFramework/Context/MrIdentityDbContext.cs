...
using Meteors.AspNetCore.Infrastructure.EntityFramework.Builder;
...
namespace Meteors.AspNetCore.Infrastructure.EntityFramework.Context
{
    ...
    where TUserToken : IdentityUserToken<TKey>
    {
        ...
        public MrIdentityDbContext(DbContextOptions options) : base(options)
        {
            ...
            this.MrTranslate = this.GetService<IMrTranslate>(isInternal: true);
        }
        ...
        public virtual TransferredProp? CurrentTransferredProps
        {
        ...
        {
            var deletedTracker = ChangeTracker.Entries().Where(entry => entry.State == EntityState.Unchanged
                 && (entry is IDeletable s && s.DateDeleted.HasValue));
                       //  entry.ToEntity<IDeletable>().DateDeleted.HasValue
        ...
        /// <summary>
        ///  open transaction to execute action inside it 
        /// </summary>
        /// <typeparam name="TEntity"></typeparam>
        /// <param name="action"></param>
        /// <returns></returns>
        public async Task<TEntity> TransactionAsync<TEntity>(Func<TEntity> action) where TEntity : class
        {
        ...
        public TEntity Transaction<TEntity>(Func<TEntity> action) where TEntity : class
        {
        ...
        public async Task TransactionAsync(Action action)
        {
            using (var tran = base.Database.BeginTransaction())
            {
                try
